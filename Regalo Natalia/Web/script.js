...
async function cargarDatosDesdeSupabase() {
    // 1. Cargar HP
    const { data: estado } = await _supabase.from('estado_jugadores').select('*').eq('id', 'partida_principal').single();
    if (estado) {
...
// --------------------------------------------------
// TIEMPO REAL (WEBSOCKETS DE SUPABASE)
// --------------------------------------------------
